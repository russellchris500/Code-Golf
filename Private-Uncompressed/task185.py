def p(i):
 i=[[0if t==max(i[0])else t for t in t]for t in i];i=[t for t in i if max(t)>0];i=[[*t]for t in zip(*i)];i=[t[::-1]for t in i];i=[t for t in i if max(t)>0];i=[t[::-1]for t in i];i=[[*t]for t in zip(*i)];t=[t[:]for t in i];i=[[t[0][0],0,t[0][3]],[0,0,0],[t[3][0],0,t[3][3]]]
