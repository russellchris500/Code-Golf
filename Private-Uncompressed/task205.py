def p(n):
 ...
 while z and z[0].count(o)*2<=l:z.pop(0);y-=1
 while z and z[-1].count(o)*2<=l:z.pop();y-=1
 if not z:return[]
 k=list(zip(*z))
 while k and k[0].count(o)*2<=len(z):k.pop(0)
 while k and k[-1].count(o)*2<=len(z):k.pop()
 if not k:return[]
