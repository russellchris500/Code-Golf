 ...
 for y in h(30):
  for d in h(30):
   if b[y][d]!=f and(y,d)not in t:
    ...
    n.append(m)
 t=min((y for y in n if len(y)>1),key=len);y,d=zip(*t);e,x=(min(y)+max(y))//2,(min(d)+max(d))//2;U=b[e][x];s={(y-e,d-x):b[y][d]for(y,d)in t};g={*t};e=[(y,d)for y in h(30)for d in h(30)if b[y][d]==U and(y,d)not in g]
 for(y,d)in e:
