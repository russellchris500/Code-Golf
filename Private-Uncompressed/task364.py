def p(e,c=range):
 ...
 for t in y:
  p=[s for(t,s)in t]
  if p:z,p,y,a=min(s[0]for s in p),max(s[0]for s in p),min(s[1]for s in p),max(s[1]for s in p);n,p=p-z+1,a-y+1;o=[[e[z+s][y+t]if 0<=z+s<d and 0<=y+t<m else 0for t in c(p)]for s in c(n)];l=[[o[s][t]for t in c(1,p-1)]for s in c(1,n-1)]if n>2and p>2else[];z={*sum(l,[])};3in z and u.append(t);len(t)==n+p-1and i.append(t)
