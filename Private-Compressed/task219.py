def p(x):
 ...
 def i(b):
  o=b;u=g(o);n,k=len(o),len(o[0]);m,d={*()},[]
  for t in range(n):
   for x in range(k):
 ...
 for(d,o)in t:t=k(o);x=c(e,t);x=[{*c(x,o)[1]}for o in y];d={*o};p=[(o,len(o&d),min(o for(o,a)in o)if o else u('inf'))for o in x];m=max(p,key=lambda k:(k[1],-k[2]));g=max(o for(a,o)in o);b.append((m[0]if m[1]>0else d,g))
 i=[o[:]for o in a]
 for(p,j)in b:
  for(y,r)in p:
   if r<=j:continue
