def p(z):
 ...
 m=sorted(T,key=lambda k:(-T[k],k));g,k=m[0],(m[1]if len(m)>1else m[0]);X={(i,j)for i in R(I)for j in R(J)if C[i][j]==g};Y={(i,j)for i in R(I)for j in R(J)if C[i][j]==k};A=[x for x in O if x[0]not in{g,k}]or O
