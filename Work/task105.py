...
def p(b,i=len,a=range):
 o=[A[:]for A in b]
 for B in a(4):
  b=f(b);o=[list(A)for A in f(o)];F,g=i(b),i(b[0]);A=[(A,B)for A in a(F)for B in a(g)if b[A][B]>0]
  if A:
   ...
   for B in a(F-1):
    for A in a(g-2):
     ...
     if i(E)>3:
      if b[B][A]>0and b[B][A+2]>0:o[B][A+1]=2
      if E and min(E)<A+1<max(E)and b[B][A+1]<1:o[B][A+1]=2
    if B in(G,H):
