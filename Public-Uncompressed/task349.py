def p(I):
	A=[A[:]for A in I];B,D=len(A),len(A[0]);from collections import Counter as a,deque;T=a(sum(A,[])).most_common()[0][0];M=[[0]*D for A in range(B)];U=[]
	for E in range(B):
		for C in range(D):
			if M[E][C]:continue
			M[E][C]=1;J=A[E][C]
			...
			while S:
				...
				for V in(-1,0,1):
					for W in(-1,0,1):
						...
						G,H=K+V,L+W
						if 0<=G<B and 0<=H<D and not M[G][H]and A[G][H]==J:M[G][H]=1;S.append((G,H));F.append((G,H))
			U.append((J,F))
	for E in range(B):
		for C in range(D):
			if A[E][C]==9:
