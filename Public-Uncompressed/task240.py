...
def _build_coarse(grid):
	...
	D,E=(A-1)//2,(B-1)//2;F=[[0]*E for A in range(D)]
...
def _fill_segments(coarse,edge):
	F=edge;A=coarse
	if A is _A:return
	H,O=len(A),len(A[0]);P=Counter(A for B in A for A in B if A);Q=[A for(A,B)in P.items()if B>=8];G=[A[:]for A in A]
	for B in Q:
		for C in range(H):
